...
{
    public partial class rep4 : Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            using (var db = Database.SPBase())
            {
                ...
                var k_list = new List<KagentComboBoxItem>() { new KagentComboBoxItem { KaId = 0, Name = "Усі" } }.Concat( db.Kagent.Where(w=> w.Deleted == 0 && (!w.Archived.HasValue || w.Archived == 0)).OrderBy(o => o.Name).Select(s => new KagentComboBoxItem { KaId = s.KaId, Name = s.Name })).ToList();
