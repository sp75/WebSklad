    ...
    public class CustomerReturnsRepository : BaseRepository
    {
        ...
        public List<Guid> GreateReturnToSupplier(Guid customer_id)
        {
            ...
            using (var sp_base = SPDatabase.SPBase())
            {
                ...
                var ka = sp_base.v_Kagent.AsNoTracking().Where(w => w.Id == customer_id).Select(s => new { s.EnterpriseId, s.WId }).FirstOrDefault();
                ...
                if (/*error == 0 && */sp_base.WaybillDet.Any(a => a.WbillId == _wb.WbillId))
                {
                    ...
                    var cor_doc = new ExecuteWayBill().CorrectDocument(_wb.WbillId, $"Корегування віддаленого повернення товрів по документу №{_wb.Num}", true);
