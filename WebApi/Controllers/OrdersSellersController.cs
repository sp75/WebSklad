  ...
  where  wbl.OnDate between {2} and GETDATE()
        ...
        public IHttpActionResult CreateReturned()
        {
            using (var sp_base = SPDatabase.SPBase())
            {
                ...
                foreach (var pos_out in GetPosOut(Context.Token.Value))
                {
                    bool stop = false;
                    int num = 1;
                    decimal amount = pos_out.RemoteAmount;

                    foreach (var pos_in in GetPosIn(pos_out.PosId).Where(w => w.Remain > 0))
                    {
                        if (!stop)
                        {
        ...
        private List<GetPosInView> GetPosIn(int pos_in)
        {
            using (var sp_base = SPDatabase.SPBase())
            {
                return sp_base.Database.SqlQuery<GetPosInView>(@"
        ...
        private List<GetPosOutView> GetPosOut(Guid customer_id)
        {
            using (var sp_base = SPDatabase.SPBase())
            {
                return sp_base.Database.SqlQuery<GetPosOutView>(@"
   select wbd.PosId, 
		 wbl.OnDate, 
         wbd.Amount, 
		 wbd.Price, 
